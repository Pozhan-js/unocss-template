            ...
            <template v-if="item.name">
              <view
                ...
                <view class="item-label">
                  <view
                    class="item-icon uni-inline-item flex-center"
                  ...
                  <view
                    class="uni-flex-item uni-inline-item flex-center flex-1"
...
export default {
  ...
  methods: {
    ...
    _onItemSelect(item, index) {
      if (!this.selectParent && item.childCount > 0) {
      ...
      if (!this.multiple) {
        ...
        this._hide();
        this.$emit("select-change", selectedList, selectedNames, index);
        return;
      }
    ...
    _setCheckKeys(val = []) {
      ...
      if (Array.isArray(val) && val.length) {
        // 默认选中的数组(使用深拷贝防止删除item时改变原值)
        let checkArr = JSON.parse(JSON.stringify(val));
        // 收集匹配的数组
        let selectArr = [];
        // 解构赋值获取需要参数
        let { childrenKey: cKey, treeList } = this;
        // 执行递归函数
        deep(treeList, checkArr);
        function deep(list, arr) {
          // 当默认选中的数组都匹配完被移除时,直接结束匹配
          if (!arr.length) return;
          for (const _key in list) {
            let item = list[_key];
            let index = arr.indexOf(item.id);
            if (index !== -1) {
              arr.splice(index, 1);
              selectArr.push({ index: _key >> 0, item });
            }
            if (Array.isArray(val[cKey])) deep(val[cKey], arr);
          }
        }
        selectArr.forEach(({ item, index }) => {
          this._onItemSelect(item, index);
        });
      }
    },
  },
  watch: {
    localdata: {
      immediate: true,
      handler(val) {
        if (val.length) this._initTree();
      },
    },
    defaultCheckedKeys: {
      immediate: true,
      handler(val) {
        // watch监听的值必须有一个变量承接,固定值或者表达式计算值都会因视图更新而重新触发watch
        if (val.length) this._setCheckKeys(val);
      },
    },
    localTreeList() {
