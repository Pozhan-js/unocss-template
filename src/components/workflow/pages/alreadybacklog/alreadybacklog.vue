<template>
	<view>
		...
		<view class="list" v-if="alreadyflows.length">
			<view class="item" hover-class="uni-list-cell-hover" v-for="(item,index) in alreadyflows" :key="index"
				@tap="alreadyflow(item)">
				<view class="item-head item-part">
					<view class="uni-item-left-name font-size30 color-3A3A3A">{{item.F_FullName}}
						<!-- <text class="marks" :class="item.FlowUrgent.statecs"> --></text></view>
			...
			init() {
				return new Promise(resolve => {
					...
					getFlowBefore(2, {
						...
						this.status = this.alreadyflows.length == res.data.pagination.total ?
							"nomore" : 'loadmore'
						resolve()
					});
				})
