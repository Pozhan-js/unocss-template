		...
		return new Promise((resolve,reject)=>{
			...
			xmlRequest.onreadystatechange = ev => {
				if (xmlRequest.readyState == 4) {
					if (xmlRequest.status == 200&& JSON.parse(xmlRequest.responseText).code == 200) {
						console.log('上传完成',xmlRequest.responseText)
		...
		return new Promise((resolve,reject)=>{
			...
			form['success'] = res => {
				if (res.statusCode == 200) {
					console.log('上传完成,微信端返回不一定是字符串，根据接口返回格式判断是否需要JSON.parse：' + res.data)
