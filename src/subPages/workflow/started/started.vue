  ...
  <view>
    ...
    <view class="list" v-if="starflows.length">
      <u-swipe-action>
        <u-swipe-action-item
          ref="swipeActionItem"
          v-for="(item, index) in starflows"
          :options="options"
          :key="index"
          @click="removeDetils(item, index)">
          <view class="item" @click.stop="starflow(item)">
            ...
            <view class="item-foot item-part">
              ...
              <view
                class="font-size24"
                :class="getStatusObj(item.status).statecss">
...
<script>
import { delFlowLaunch } from "@/api/work.js";
import helper from "@/common/helper.js";
import { getFlowStartList } from "@/api/mongoApi/work";
    ...
    init() {
      return new Promise(resolve => {
        ...
        getFlowStartList({
          ...
          this.status =
            this.starflows.length == res.data.pagination.total
              ? "nomore"
              : "loadmore";
          resolve();
        });
